# SVM, LR, NB, Bagging and Fuzzy (With smote) (optimal)
...
for random_state in range(40, 45):
    ...
    best_svm, svm_best_params = grid_search(SVC(random_state=random_state), svm_params, X_train_scaled, y_train_smote)
    best_svm_1, svm_best_params_1 = grid_search(SVC(probability=True, random_state=random_state), svm_params, X_train_scaled, y_train_smote)
    ...
    best_lr, lr_best_params = grid_search(LogisticRegression(max_iter=1000, random_state=random_state), lr_params, X_train_scaled, y_train_smote)
    ...
    best_nb, nb_best_params = grid_search(GaussianNB(), nb_params, X_train_scaled, y_train_smote)
...
###############################################
    ...
    for train_idx, test_idx in skf.split(X, y):
        ...
        scaler = StandardScaler()
        X_train_cv_scaled = scaler.fit_transform(X_train_resampled)
        X_test_cv_scaled = scaler.transform(X_test_cv)
        
        # Train each model
        svm_model.fit(X_train_cv_scaled, y_train_resampled)
        lr_model.fit(X_train_cv_scaled, y_train_resampled)
        nb_model.fit(X_train_cv_scaled, y_train_resampled)
...
final_df.to_csv('results_with_smote_optimal.csv')
