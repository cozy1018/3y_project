# (2)
#### Results presented in the paper (SVM)
...
print("Processing data with two classes to build SVM models, applying feature scaling, and hyperparameter tuning.")
...
features = ['age', 'sex', 'cp', 'chol', 'fbs', 'exang', 'oldpeak', 'slope', 'ca', 'num']
df = df[features]
...
grid_search_no_smote = GridSearchCV(SVC(random_state=42), param_grid, cv=10)
...
# Hyperparameter tuning using GridSearchCV
grid_search_smote = GridSearchCV(SVC(random_state=42), param_grid, cv=10)
